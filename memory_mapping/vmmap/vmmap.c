...
static int my_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	for (i = 0; i < NPAGES; i++) {
        ...
        // 使用 remap_pfn_range 进行页映射
        ret = remap_pfn_range(vma, start, pfn, PAGE_SIZE, vma->vm_page_prot);
        if (ret < 0) {
            pr_err("remap_pfn_range failed on page %d\n", i);
            return ret;
        }
...
static int __init my_init(void)
{
	...
	for (i = 0; i < NPAGES; i++) {
        struct page *page = vmalloc_to_page(vmalloc_area + i * PAGE_SIZE);
        SetPageReserved(page);  // 标记为保留
        memset(vmalloc_area + i * PAGE_SIZE, 0, PAGE_SIZE);  // 初始化页面内容
